...
export default function ProductSellerUI(props) {
  return (
    <>
      ...
      <input
        type="text"
        onChange={props.onChangeMySeller}
        defaultValue={props.data?.fetchProduct.myseller}
      ></input>
      상품명:{" "}
      <input
        type="text"
      ...
      ></input>
      {!props.isEdit && (
        <Button onClick={props.onClickSummit} qqq={props.qqq}>
          상품등록하기
        </Button>
      )}
      {props.isEdit && (
        <Button onClick={props.onClickEdit} qqq={props.qqq}>
          상품수정하기
        </Button>
      )}
    </>
...
//수정페이지 기본값 설정 defaultValue={props.data?.fetchproduct.myseller}
